                ...
                <div class="card mb-4">
                    <div class="card-header">
                        ...
                        <span class="float-end">
                            ...
                            <button class="btn btn-primary btn-sm" href="javascript::" data-bs-toggle="modal" data-bs-target="#CreateUser"><i class="me-1" data-feather="folder-plus"></i> &nbsp;Opret kunde</button>
                        @endif  
                            </div>
                                ...
                                <div class="modal-dialog modal-lg  modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header bg-light">
                                            <h5 class="modal-title" id="exampleModalLabel"><i class="me-1" data-feather="folder-plus"></i> Opret kunde</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form method="post" action="{{ url('/customer/store') }}" enctype="multipart/form-data">
                                                @csrf
                                                ...
                                                <div class="row gx-3 mb-3">
                                                    ...
                                                    <div class="col-md-4">
                                                        ...
                                                        <input class="form-control" type="text" name="employment_date" value="" />
                                                    </div>
                                                </div>
                                ...
                                @foreach ($customers as $customer)
                                    <tr>
                                        <td style="width: 50px;" align="center">
                                            <a href="{{ url('/customer/view/'.$customer->id) }}">
                                                @if (!empty($customer->image))
                                                    <img style="width: 50px !important" class="img-thumbnail" src="{{ asset('backend/profile/'.$customer->image) }}" />
                                                @else
                                                    <img style="width: 50px !important" class="img-thumbnail" src="{{ asset('backend/placeholder.jpg') }}" />
                                                @endif
                                            </a>
                                        </td>
                                        <td style="min-width: 200px;"><a href="{{ url('/customer/view/'.$customer->id) }}">{{ $customer->name }}</a></td>
                                        
                                        <td>
                                            @if ($customer->phone)
                                                <div><i class="dropdown-item-icon" style="color: #69707A !important; height: 0.95em; width: 0.95em;" data-feather="phone"></i>&nbsp; {{ $customer->phone }}</div>
                                            @endif
                                           {{--  @if ($customer->bank_information)
                                                <div style="padding-top: 5px;"><i class="dropdown-item-icon" style="color: #69707A !important; height: 0.95em; width: 0.95em;" data-feather="phone"></i>&nbsp; {{ $customer->bank_information }}</div>
                                            @endif --}}
                                            @if ($customer->email)
                                                <div style="padding-top: 5px;"><i class="dropdown-item-icon" style="color: #69707A !important; height: 0.95em; width: 0.95em;" data-feather="mail"></i>&nbsp; <a href="mailto:{{ $customer->email }}">{{ $customer->email }}</a></div>
                                            @endif
                                        ...
                                        <td>
                                            @php
                                                error_reporting(0);
                                                $locations_ids0 = $customer->location_id;
                                        ...
                                        <td>{{ $customer->bank_information }}</td>
    ...
    $(function () {
        ...
        new simpleDatatables.DataTable("#staffDatatable", {
            labels: {
                ...
                perPage: "Kunder pr. side &nbsp;  {select}",
                noRows: "Ingen kunder matchede din søgning!",
                info: "Viser {start} til {end} af {rows} kunder",
